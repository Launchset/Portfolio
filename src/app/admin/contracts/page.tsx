...
export default async function ContractsPage() {
  ...
  const result = await APP_DB.prepare(
    `SELECT contracts.id, contracts.title, contracts.status, contracts.signed_at, clients.id AS client_id, clients.name AS client_name, clients.email AS client_email FROM contracts JOIN clients ON clients.id = contracts.client_id WHERE clients.archived_at IS NULL ORDER BY contracts.created_at DESC`,
  ).all<ContractRow>();
    ...
    <>
      <section className={styles.pageHeader}>
        <div>
          <span>ADMIN</span>
          <h1>Contracts</h1>
          <p>Pending and completed client agreements.</p>
        </div>
      </section>
