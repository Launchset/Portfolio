...
export default async function ClientPage({
  ...
  const client = await APP_DB.prepare(
    `SELECT clients.*, contracts.id AS contract_id, contracts.title AS contract_title, contracts.status AS contract_status FROM clients LEFT JOIN contracts ON contracts.client_id = clients.id WHERE clients.id = ? AND clients.archived_at IS NULL ORDER BY contracts.created_at DESC LIMIT 1`,
  )
    .bind(id)
    .first<ClientDetail>();
  if (!client) notFound();
      ...
      <div className={styles.detailGrid}>
        <section className={styles.detailPanel}>
          ...
          <dl>
            <div>
              <dt>Status</dt>
              <dd>{client.status.replaceAll("_", " ")}</dd>
            </div>
            <div>
              <dt>Maintenance</dt>
              <dd>
            ...
            <div>
              ...
              <dd>
                {client.invited_at
                  ? new Date(client.invited_at).toLocaleDateString("en-GB")
                  : "Not sent"}
              </dd>
            </div>
          </dl>
        ...
        <section className={styles.detailPanel}>
          ...
          {client.contract_id ? (
            <>
              <p>{client.contract_title}</p>
              <Link href={`/admin/contracts/${client.contract_id}`}>
        ...
        <section className={styles.detailPanel}>
          ...
          <Link href={`/admin/payments/${client.id}`}>View payment</Link>
        </section>
      </div>
    </>
  );
