...
function PaymentForm() {
  ...
  if (result.type === "loading")
    return <div className={styles.loading}>Loading secure payment form…</div>;
  if (result.type === "error")
    return <div className={styles.error}>{result.error.message}</div>;
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(
        error instanceof Error && error.message === "confirmation_timeout"
  ...
  return (
    <form className={styles.paymentForm} onSubmit={submit}>
      <div className={styles.checkoutGuide}>
        ...
        <p>
          Stripe securely keeps this payment method for your monthly maintenance
          subscription. Launchset does not receive or store your card details.
        </p>
        <p>
          Additional information is optional unless Stripe marks a field as
          required. You can change the country and mobile dialling code before
          paying.
        </p>
      </div>
      <PaymentElement
      ...
      <label className={styles.linkChoice}>
        ...
        <span className={styles.linkChoiceCopy}>
          ...
          <small>
            Optional — use Stripe Link to reuse your details in the future.
          </small>
        </span>
      </label>
      {saveForFasterCheckout && (
        <p className={styles.linkDetails}>
          Stripe may ask for your email and mobile number to create or sign in
          to Link. You can turn this option off before paying.
        </p>
      )}
      {message && (
      ...
      <button
        aria-busy={busy}
        disabled={busy || !result.checkout.canConfirm}
        type="submit"
      >
        {busy ? "Confirming with Stripe…" : "Start monthly maintenance"}
      </button>
      <p className={styles.secureNote}>
        Payment details are encrypted and handled directly by Stripe.
      </p>
    </form>
  );
}

export default function LaunchsetCustomCheckout({
  publishableKey,
}: {
  ...
  const [clientSecret] = useState(() =>
    fetch("/api/billing/checkout", { method: "POST" }).then(
      async (response) => {
        ...
        if (!response.ok || !payload.clientSecret)
          throw new Error(
            payload.error ?? "Stripe Checkout could not be opened.",
          );
